...
namespace ServipagMobile.Droid {
	public class FragmentIngresaDatosUPDT : Fragment {
		...
		public FragmentIngresaDatosUPDT() { }

		public FragmentIngresaDatosUPDT(PDUActivity PDUAct, FragmentManager sfm, PaseTardio uPDT, bool isLogin, string idBiller, string idServicio) {
			this.PDUAct = PDUAct;
			this.sfm = sfm;
			this.uPDT = uPDT;
		...
		public override void OnViewCreated(View view, Bundle savedInstanceState) {
			...
			infoFechaUPDU.Click += (sender, e) => {
				PDUAct.changeMainFragment(new FragmentTCAutopista(PDUAct), "tcPDU");
			};
		...
		public async void validaPatente(JObject parametros) {
			...
			if (respuesta.Success) {
				if ((int)respuesta.State["Error"] == 0) {
					...
					PDUAct.pd.patente = uPDT.patente;
					PDUAct.pd.fecha_circulacion = hintFechaUPDU.Text;
					PDUAct.pd.categoria = Int32.Parse(getCategoria());
					PDUAct.pd.isPDU = uPDT.isPDU;
					PDUAct.pd.idBiller = uPDT.idBiller;
					PDUAct.pd.idServicio = uPDT.idServicio;
					PDUAct.pd.tipoPDU = uPDT.tipoPDU;
		...
		private void setValuesUPDU() {
			typeSelect = uPDT.tipoPDU;
			setTypeSelected();
			patente.Text = uPDT.patente;
		}
		...
		private void setTypeSelected() {
			...
			ttlTipoUPDU.Text = Resources.GetString(Resource.String.pdu_title_nacional);
			selectCategoryUPDU.SetCurrentItem(uPDT.categoria - 1, true);
			break;
			case "motos":
			imgNacionalUPDU.SetImageDrawable(Resources.GetDrawable(Resource.Drawable.pdu_nacional_off));
			imgMotosUPDU.SetImageDrawable(Resources.GetDrawable(Resource.Drawable.pdu_motos_on));
