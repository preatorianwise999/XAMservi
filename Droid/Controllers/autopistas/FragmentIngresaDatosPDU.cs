...
namespace ServipagMobile.Droid {
	public class FragmentIngresaDatosPDU : Fragment {
		...
		public FragmentIngresaDatosPDU(PDUActivity PDUAct, FragmentManager sfm, bool isLogin, string idBiller, string idServicio) {
			this.PDUAct = PDUAct;
		...
		public override void OnViewCreated(View view, Bundle savedInstanceState) {
			...
			imgNacional.Click += (sender, e) => {
				if (!typeSelect.Equals("nacional")) {
					...
					fieldRePatente.ImeOptions = Android.Views.InputMethods.ImeAction.Next;
			...
			imgMotos.Click += (sender, e) => {
				if (!typeSelect.Equals("motos")) {
					...
					fieldRePatente.ImeOptions = Android.Views.InputMethods.ImeAction.Done;
		...
		public async void validaPatente(JObject parametros) {
			...
			if (respuesta.Success) {
				if ((int)respuesta.State["Error"] == 0) {
					if (respuesta.Response["retorno"].ToString() == "0") {
